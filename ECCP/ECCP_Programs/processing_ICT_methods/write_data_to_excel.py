...
electronic_coupling_names = ('Hole Coupling','Electron Coupling')
def write_data_to_excel(eigendata, ict_data_foldername, individual_ict_data_foldername, start_time):
    """
    ...
    ----------
    eigendata : list
        This is a list of all information about the dimer extracted from the eigendata, such as hole and electron tranfer energy between monomers.
    ict_data_foldername : str.
        This is the folder to place the excel files in, as well as other files created. 
    individual_ict_data_foldername : str.
    ...
    if len(eigendata) > 0:
        ...
        print('Making Excel Spreadsheet and text files with ICT Data.')
        print('Running time (HH:MM:SS): '+str(timedelta(seconds=time.time() - start_time))+' ('+str(dt_string)+')')

        # Next, reorganise the data into another format. look at this later. 
        organised_data = {}
        for data_information, (root, electronic_details) in eigendata.items():
            crystal_name, dimer_name, functional_and_basis_set_name = data_information
            ...
            organised_data[crystal_name][dimer_name][functional_and_basis_set_name] = (root, electronic_details)

        # -------------------------------------------------------------
        # Print data for ict_data_foldername

        print('Writing data to excel and text files.')
        # General variables for writing data to excel and text files. 
        
        path_to_excel_file = 'ICT_Data'

        # Set up the workbook for recording data to excel spreadsheet.
        workbook = Workbook(ict_data_foldername+'/'+path_to_excel_file+'.xlsx')

        # Set up the textfile for reporting data to.
        dataTXT_energy     = open(ict_data_foldername+'/'+path_to_excel_file+'_energy.txt', 'w')
        dataTXT_wavenumber = open(ict_data_foldername+'/'+path_to_excel_file+'_wavenumber.txt', 'w')
        ...
        write_functional_and_basis_set_sheet_data(functional_and_basis_set_data, workbook, path_to_excel_file, ict_data_foldername)
        ...
        # -------------------------------------------------------------
        # Print data for individual_ict_data_foldername

        write_individual_results(organised_data, individual_ict_data_foldername)
    ...
    for crystal_name, crystal_data in sorted(organised_data.items(), key=lambda x: [value.lower() for value in x[0].split('_')]):
        ...
        for dimer_name, dimer_data in sorted(crystal_data.items(), key=lambda x: int(x[0].split('_')[0].replace('Dimer',''))):
            ...
            for index_col in range(len(electronic_coupling_names)):
                name = electronic_coupling_names[index_col]
                if index_col == 6:
                    worksheet_energy    .write(counter_row,counter_col+index_col+1,str(name))
                    worksheet_wavenumber.write(counter_row,counter_col+index_col+1,str(name))
                else:
                    worksheet_energy    .write(counter_row,counter_col+index_col+1,str(name)+' (meV)')
                    worksheet_wavenumber.write(counter_row,counter_col+index_col+1,str(name)+' (cm-1)')
            counter_row += 1
