...
def _format_freq_Gaussian_for_franck_condon_calc(output_type):
    output_string = ""
    output_string = str(output_type)+" freq=(fc,readfc,readfcht) ! Obtain frequencies to determine if the excited state is good."
    return output_string
...
def write_gaussian_in(fd, atoms, properties=['energy'],
    ...
    #######################################################
    # ADDED BY GRW
    out.append(_format_freq_Gaussian_for_franck_condon_calc(output_type))
    ...
    if molecule_name is None:
        header_of_file = 'Gaussian input prepared by ASE: Franck-Condon Factor Job'
    else:
        header_of_file = 'Franck-Condon Factor Job for '+str(molecule_name)
