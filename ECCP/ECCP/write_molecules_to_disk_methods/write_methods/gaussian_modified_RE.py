...
Cerated methods:
* _format_nosymm_Gaussian_calc
* _format_opt_Gaussian_calc
* _format_TD_Gaussian_calc
...
def _format_opt_Gaussian_calc(output_type, perform_CalcAll=False):
    output_string = ""
    if not perform_CalcAll:
        output_string = str(output_type)+" opt ! Optimise the structure"
    else:
        output_string = str(output_type)+" opt(CalcAll) ! Optimise the structure and calculate the full Hessian at each opt step. This also performs vibrational frequency analysis automatically"
    return output_string

def _format_TD_Gaussian_calc(output_type, td_settings):
    if td_settings is not None:
        output_string = str(output_type)+" "+str(td_settings)+" ! Optimise/Obtaining excited state structure/energy (depending if opt has been given or not.)"
    else:
        output_string = str(output_type)+" td=(nstates=5,root=1) ! Optimise/Obtaining excited state structure/energy (depending if opt has been given or not.)"
    return output_string

def _format_freq_Gaussian_calc(output_type, perform_raman):
    output_string  = str(output_type)+" freq=(savenormalmodes,hpmodes"
    if perform_raman:
        output_string += ",raman"
    output_string += ") ! Obtain frequencies to determine if the excited state is good." 
    return output_string

def _format_density_Gaussian_calc(output_type):
    output_string  = str(output_type)+" density=current"
    return output_string

def _format_population_analysis_Gaussian_calc(output_type):
    output_string = ""
    output_string = str(output_type)+" pop=Full ! Obtain printouts of all the molecular orbitals and orbital energies. Needed for obtaining Franck-Condon and Huang-Rhys parameters."
    return output_string

def _read_chk_file_method(output_type):
    output_string = ""
    #output_string = str(output_type)+" Geom=Checkpoint Guess=Read"
    output_string = str(output_type)+" Geom=Check Guess=TCheck ! Will read in the geometry and electronic details from the checkpoint file"
    return output_string
...
def write_gaussian_in(fd, atoms, perform_opt, perform_CalcAll, perform_TD, 
                      perform_freq, perform_raman, perform_density, 
                      perform_pop, properties=['energy'], # read_in_oldchk_file
                      method=None, basis=None, fitting_basis=None,
                      output_type=None, basisfile=None, basis_set=None,
                      xc=None, charge=None, mult=None, extra=None,read_chk_file=False,
                      ioplist=None, addsec=None, spinlist=None,
                      zefflist=None, qmomlist=None, nmagmlist=None,
                      znuclist=None, radnuclearlist=None, 
                      td_settings=None, molecule_name=None,
                      **params):
    '''
    ...
    # ADDED BY GRW
    if perform_opt:
        out.append(_format_opt_Gaussian_calc(output_type, perform_CalcAll=perform_CalcAll))
    if perform_TD:
        out.append(_format_TD_Gaussian_calc(output_type, td_settings))
    if perform_freq:
        out.append(_format_freq_Gaussian_calc(output_type, perform_raman))
    if perform_density:
        out.append(_format_density_Gaussian_calc(output_type))
    if perform_pop:
        out.append(_format_population_analysis_Gaussian_calc(output_type))
    ...
    if molecule_name is None:
        header_of_file = 'Gaussian input prepared by ASE: Reorganisation Energy Job'
    else:
        header_of_file = 'Reorganisation Energy Job for '+str(molecule_name)
