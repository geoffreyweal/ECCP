"""
gaussian_modified.py, Geoffrey Weal, 22/2/22

gaussian_modified is designed to save the gaussian .gjf to file. 

This file has been modified from https://gitlab.com/ase/ase/-/blob/master/ase/io/gaussian.py to allow the user to include fragements in the .gjf file.

Methods that have been modifed are:
    * write_gaussian_in

Cerated methods:
* _format_TrEsp_Gaussian_calc
* _format_ONIOM_method
* _write_wfn_output_name
* _read_chk_file_method
* _format_get_overlap_matrix_and_MOs
* _add_read_checkpoint
...
def _get_molecule_spec(atoms, nuclear_props, environment_copy, no_atoms_without_environment):
    ...
    for i, atom in enumerate(atoms):
        ...
        oniom_add = '' if ( environment_copy == Atoms()) else (' H' if (i < (len(atoms) - no_atoms_without_environment)) else ' L')
        molecule_spec.append('{:<10s}{:20.10f}{:20.10f}{:20.10f}{}'.format(symbol_section, *atom.position, oniom_add))
...
def _format_ONIOM_method(output_type, method, basis, fitting_basis, td_settings):

    # Get functional and basis set 
    if fitting_basis:
        func_and_basis_set = '{}/{}/{}'.format(method, basis, fitting_basis)
    else:
        func_and_basis_set = '{}/{}'.format(method, basis)
    
    # Get the TD data
    if td_settings is not None:
        td_string = str(td_settings)
    else:
        td_string = 'TD'

    # Write string and return
    output_string = str(output_type)+' ONIOM('+str(func_and_basis_set)+'/'+str(td_string)+':'+str(func_and_basis_set)+')'
    output_string += " density=(transition=1) nosymm output=wfn ! This line will perform a TD calc to be outputted to a wfn file. This can be read into Multiwfn."
    return output_string

def _format_TrEsp_Gaussian_calc(output_type, td_settings=None):
    output_string = str(output_type)
    if td_settings is not None:
        output_string += ' '+str(td_settings)
    else:
        output_string += ' TD'
    output_string += " density=(transition=1) nosymm output=wfn ! This line will perform a TD calc to be outputted to a wfn file. This can be read into Multiwfn."
    return output_string

def _format_get_overlap_matrix(output_type, obtain_overlap_matrix=False):
    if obtain_overlap_matrix:
        output_string = str(output_type)+" iop(3/33=1) ! This will provide the information for obtaining the overlap matrix "
    else:
        output_string = ''
    return output_string

def _format_get_excitation_amplitudes(output_type, obtain_excitation_amplitudes=False):
    if obtain_excitation_amplitudes:
        output_string = str(output_type)+" iop(9/40=5) ! This will provide the excitation amplitudes for your compound."
    else:
        output_string = ''
    return output_string

def _write_wfn_output_name(wfn_filename):
    output_string = ""
    output_string = str(wfn_filename)+ ' ! This is the name of the wfn file'
    return output_string

def _read_chk_file_method(output_type):
    output_string = ""
    #output_string = str(output_type)+" Geom=Checkpoint Guess=Read"
    output_string = str(output_type)+" Geom=Check Guess=Read ! Will read in the geometry and electronic details from the checkpoint file"
    return output_string
...
def modify_link0_list_for_link1(params, link0_list):
    return params, link0_list

def _format_nosymm_Gaussian_calc(output_type):
    output_string = ""
    output_string = str(output_type)+" nosymm ! "
    return output_string

def _format_Int_grid(output_type):
    output_string = ""
    #output_string = str(output_type)+" Geom=Checkpoint Guess=Read"
    output_string = str(output_type)+" Int=UltraFine ! This is default in G16, but here if calcs are run on other Gaussian. This splits the intergration grid into very tiny pieces (99,590 grid points)."
    return output_string

def _format_get_MOs(output_type, obtain_MO_energies_and_coefficients=False):
    if obtain_MO_energies_and_coefficients:
        output_string = str(output_type)+" punch=mo ! This will provide the information for obtaining the MO energies and coefficients"
    else:
        output_string = ''
    return output_string

def _read_pop_analysis_method(output_type, pop_type='full'):
    output_string = ""
    output_string = str(output_type)+" pop="+str(pop_type)+' ! This will give the MO coefficients in the output.log file. This is not needed as the orbitals that go with each atoms are given in a ATC calculation. However, if this is not given, note that the natural orbtial coefficients will be given rather than the MO coefficients in the output.log file. This is fine, as the MO coefficients will be read from the fort.7 file.'
    return output_string
...
def write_gaussian_in(fd, atoms, environment=None, properties=['energy'],
                      method=None, basis=None, fitting_basis=None,
                      output_type=None, basisfile=None, basis_set=None,
                      xc=None, charge=None, mult=None, extra=None,read_chk_file=None,
                      ioplist=None, addsec=None, spinlist=None,
                      zefflist=None, qmomlist=None, nmagmlist=None,
                      znuclist=None, radnuclearlist=None, td_settings=None, 
                      obtain_overlap_matrix_and_MO_coefficients=False,
                      obtain_excitation_amplitudes=False,
                      wfn_filename='output.wfn',
                      molecule_name=None,
                      **params):
    '''
    ...
    original_environment : Atoms or None
        This is the environment, consisting of the molecules that surround the molecule. 
    properties: list
    ...
    radnuclearlist: list
        ...
        propeties section of the molecule specification.
    molecule_name : str
        This is the name of the molecule (monomer). Default: None
    params: dict
    ...
    # Make a copy of the original atoms and environment
    atoms_copy       = atoms.copy()
    if (environment is not None):
        raise Exception('ERROR: NEED TO DEVELOP METHOD FOR ATCS WITH ENVIRONMENT')
    environment_copy = environment.copy() if (environment is not None) else Atoms()

    # Make sure the periodic boundary settings is turnned off. 
    atoms_copy.set_pbc(False)
    environment_copy.set_pbc(False)
    ...
    if charge is None:
        charge = (atoms_copy+environment_copy).get_initial_charges().sum()

    # determine multiplicity from initial magnetic moments
    # if not passed explicitly
    if mult is None:
        mult = (atoms_copy+environment_copy).get_initial_magnetic_moments().sum() + 1

    # set up link0 arguments
    out = []
    params_second = {}
    if 'mem' in params:
        params_second['mem'] = str(params['mem'])
    if 'nprocshared' in params:
        params_second['nprocshared'] = str(params['nprocshared'])
    params_second['oldchk'] = str(params['chk'])
    ...
    #######################################################
    # ADDED BY GRW
    if environment_copy == Atoms():
        out.append(_format_method_basis(output_type, method, basis, fitting_basis))
        out.append(_format_TrEsp_Gaussian_calc(output_type, td_settings))
    else:
        out.append(_format_ONIOM_method(output_type, method, basis, fitting_basis, td_settings))
    if obtain_overlap_matrix_and_MO_coefficients:
        out.append(_format_get_overlap_matrix(output_type, obtain_overlap_matrix=obtain_overlap_matrix_and_MO_coefficients))
    if obtain_excitation_amplitudes:
        out.append(_format_get_excitation_amplitudes(output_type, obtain_excitation_amplitudes=obtain_excitation_amplitudes))
    if read_chk_file:
        out.append(_read_chk_file_method(output_type))
    #######################################################
    ...
    format_route_params = _format_route_params(params)
    if not format_route_params == []:
        out.extend(format_route_params)
    ...
    # Get the header for this Gaussian Job
    if molecule_name is None:
        header_of_file = 'Gaussian input prepared by ASE: ATC Job'
    else:
        header_of_file = 'ATC Job for '+str(molecule_name)
    if not (len(environment_copy) == 0):
        header_of_file += '; No of atoms in molecule = '+str(atoms_copy)

    # header, charge, and mult
    #import pdb; pdb.set_trace()
    out += ['', header_of_file, '', '{:.0f} {:.0f}'.format(charge, mult)]
    ...
    molecule_spec = _get_molecule_spec(atoms_copy+environment_copy, nuclear_props, environment_copy, len(atoms_copy))
    for line in molecule_spec:
        out.append(line)

    out.extend(_format_basis_set(basis, basisfile, basis_set))
    addsec_list = _format_addsec(addsec)
    if len(addsec_list) > 0:
        out.extend(_format_addsec(addsec))
        out += ['']

    #######################################################
    # ADDED BY GRW
    out.append(_write_wfn_output_name(wfn_filename))
    #######################################################
    ...
    #####################################################################################################################################################################
    # ADDED BY GRW
    # Link 1 address for obtaining mo energies and coefficients
    if obtain_overlap_matrix_and_MO_coefficients:
        ...
        # set up link0 arguments
        params, link0_list = _pop_link0_params(params_second)
        #params, link0_list = modify_link0_list_for_link1(params, link0_list)
        out.extend(link0_list)
        ...
        #######################################################
        # ADDED BY GRW
        out.append(_format_get_MOs(output_type, obtain_MO_energies_and_coefficients=obtain_overlap_matrix_and_MO_coefficients))
        out.append(_read_pop_analysis_method(output_type))
        out.append(_format_nosymm_Gaussian_calc(output_type))
        out.append(_format_Int_grid(output_type))
        out.append(_read_checkpoint_file_method(output_type))
        #######################################################
