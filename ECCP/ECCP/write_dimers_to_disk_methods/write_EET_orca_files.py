...
def write_EET_orca_files(original_molecule_1, original_molecule_2, full_dimer_name, gaussian_jobs_path, fragmentlist, all_gaussian_parameters_for_EETs, all_submission_information_for_EETs, get_dimer_eets=True, submit_EETs_in_parallel=True):
	"""
	...
	all_gaussian_parameters_for_EETs : list
	...
	get_dimer_eets : bool.
		This tag indicates if the user wants to obtain Gaussian files for running EET jobs on the dimers. Default: True

	submit_EETs_in_parallel : bool.
		If True, write submit.sl files for submitting individual jobs in slurm ('parallel'). If False, write a single submit.sl file for submitting this job to run Dimer jobs one after another. This should be set to false if you know your Dimer jobs will take less than 5 minutes to run each job. Default: True.  
	"""

	# ******************************************************************************************
	# This is included as a test for debugging to see if the eigendata from EET is the same as for the Eigendata calcs.
	# for index in range(len(all_gaussian_parameters_for_EETs)):
	#	all_gaussian_parameters_for_EETs[index]['show_eigendata_in_output_file'] = True
	# ******************************************************************************************
	...
	for original_gaussian_parameters, original_submission_information in zip(all_gaussian_parameters_for_EETs, all_submission_information_for_EETs):
		...
		check_against_file(dimer, path_to_dimer)

		# Fourteenth, create the gaussian .gjf file.
		with open(path_to_dimer, 'w') as fd:
			write_gaussian_in_EET(fd, molecule_1, molecule_2, run_EET=True, full_dimer_name=full_dimer_name, **gaussian_parameters)
