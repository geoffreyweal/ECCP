...
And
* _format_get_overlap_matrix
* _format_get_MOs
* _read_pop_analysis_method
...
def _format_td(output_type, td_settings=None):
    if td_settings is not None:
        output_string = str(output_type)+" "+str(td_settings)+" nosymm ! This will perform a TD-DFT calc needed for the EET algorithm"
    else:
        output_string = str(output_type)+" td(nstates=1) nosymm ! This will perform a TD-DFT calc needed for the EET algorithm"
    return output_string

def _format_eet(output_type, fragmentlist):
    output_string = ""
    output_string = str(output_type)+" eet(fragment="+str(len(set(fragmentlist)))+") ! This will calculate the exciton coupling energy for the dimer pair"
    return output_string
...
def _read_pop_analysis_method(output_type, pop_type='full'):
    output_string = ""
    output_string = str(output_type)+" pop="+str(pop_type)+' ! This will give the MO coefficients in the output.log file. THIS IS NEEDED TO OBTAIN WHICH ORBITALS GO WITH WHICH ATOMS.'
    return output_string
...
def write_gaussian_in(fd, original_molecule_1, original_molecule_2, 
                      original_environment, properties=['energy'],
                      method=None, basis=None, fitting_basis=None,
                      output_type=None, basisfile=None, basis_set=None,
                      xc=None, charge1=None, mult1=None, charge2=None, mult2=None, 
                      charge_environment=None, mult_environment=None, 
                      extra=None,read_chk_file=None,
                      ioplist=None, addsec=None, spinlist=None,
                      zefflist=None, qmomlist=None, nmagmlist=None,
                      znuclist=None, radnuclearlist=None, fragmentlist=None, 
                      run_EET=True, td_settings=None, show_eigendata_in_output_file=False,
                      obtain_excitation_amplitudes=False, full_dimer_name=None,
                      **params):
    '''
    ...
    environment : Atoms or None
        This is the environment, consisting of the molecules that surround the dimer. 
    properties: list
    ...
    radnuclearlist: list
        ...
        propeties section of the molecule specification.
    full_dimer_name : str
        This is the name of the dimer. Default: None
    params: dict
    ...
    environment = original_environment.copy() if (original_environment is not None) else Atoms()

    # Make sure the periodic boundary settings is turnned off. 
    molecule_1.set_pbc(False)
    molecule_2.set_pbc(False)
    environment.set_pbc(False)
    ...
    if charge1 is None:
        charge1 = molecule_1.get_initial_charges().sum()
    if charge2 is None:
        charge2 = molecule_2.get_initial_charges().sum()
    if charge_environment is None:
        charge_environment = environment.get_initial_charges().sum()
    total_charge = charge1 + charge2 + charge_environment

    # determine multiplicity from initial magnetic moments
    # if not passed explicitly
    if mult1 is None:
        mult1 = molecule_1.get_initial_magnetic_moments().sum() + 1
    if mult2 is None:
        mult2 = molecule_2.get_initial_magnetic_moments().sum() + 1
    if mult_environment is None:
        mult_environment = environment.get_initial_magnetic_moments().sum() + 1
    total_mult = (mult1-1) + (mult2-1) + (mult_environment-1) + 1
    ...
    #######################################################
    # ADDED BY GRW
    out.append(_format_td(output_type,td_settings))
    out.append(_format_eet(output_type,fragmentlist))
    if show_eigendata_in_output_file:
        out.append(_format_get_overlap_matrix(output_type, obtain_overlap_matrix=True))
        out.append(_read_pop_analysis_method(output_type, pop_type='full'))
    if obtain_excitation_amplitudes:
    ...
    if full_dimer_name is None:
        header_of_file = 'Gaussian input prepared by ASE: EET Job'
    else:
        header_of_file = 'EET Job for '+str(full_dimer_name)

    # header, charge, and mult
    out += ['', header_of_file, '', '{:.0f} {:.0f}'.format(total_charge, total_mult)+' '+'{:.0f} {:.0f}'.format(charge1, mult1)+' '+'{:.0f} {:.0f}'.format(charge2, mult2)]

    # make dict of nuclear properties:
    nuclear_props = {'spin': spinlist, 'zeff': zefflist, 'qmom': qmomlist,
                     'nmagm': nmagmlist, 'znuc': znuclist,
                     'radnuclear': radnuclearlist, 'fragment': fragmentlist} # 'fragment': fragmentlist added to dictionary GRW
    nuclear_props = {k: v for k, v in nuclear_props.items() if v is not None}

    # atomic positions and nuclear properties:
    molecule_spec = _get_molecule_spec(molecule_1+molecule_2+environment, nuclear_props)
