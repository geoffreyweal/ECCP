...
def write_gaussian_in(fd, atoms, properties=['energy'],
                      ...
                      get_icts=True, td_settings=None,
                      obtain_excitation_amplitudes=False,
                      full_dimer_name=None,
                      **params):
    '''
    ...
    out.append(_format_get_overlap_matrix(output_type, obtain_overlap_matrix=get_icts))
    out.append(_format_get_MOs(output_type, obtain_MO_energies_and_coefficients=get_icts))
    ...
    if full_dimer_name is None:
        header_of_file = 'Gaussian input prepared by ASE: ICT Job'
    else:
        header_of_file = 'ICT Job for '+str(full_dimer_name)
