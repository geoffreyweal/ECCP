#!/usr/bin/env python3
'''
Geoffrey Weal, get_single_point_RE_Gaussian_input_file.py, 21/4/22
...
def run_method(dirpath, optimisation_filename, single_point_filename, perform_TD, gaussian_parameters={}):
    ...
    if not did_first_opt_complete:
        raise Exception('Error in get_single_point_RE_Gaussian_input_file method. Your DFT optimisation calculation in the reorganisation energy did not complete successfully. Check this and try again.')
    ...
    with open(single_point_filename,'w') as fd:
        write_gaussian_in_RE(fd, optimised_structure, perform_opt=False, perform_CalcAll=False, perform_TD=perform_TD, perform_freq=False, perform_raman=False, perform_density=perform_TD, perform_pop=perform_TD, read_chk_file=False, **gaussian_parameters_new)
...
gaussian_parameters = eval(' '.join(sys.argv[4:]))

# Third, run method
run_method(dirpath, optimisation_filename, single_point_filename, perform_TD, gaussian_parameters=gaussian_parameters)
